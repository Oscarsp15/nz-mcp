...
``cli_output.interactive_ui_enabled()``. One more lives here, because only a
...
class ProfileWizardApp(ThemedApp[WizardResult]):
    ...
    def on_resize(self, event: events.Resize) -> None:
        ...
        The ninth trigger, and the one the audit of PR #222 asked for: shrinking a window
        mid-session is routine over SSH, and the other eight only look at start-up. Leaving
