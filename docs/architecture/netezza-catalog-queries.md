...
FROM _V_DATABASE
WHERE (? IS NULL OR DATABASE LIKE UPPER(?))
ORDER BY DATABASE;
```
...
SELECT SCHEMA, OWNER
FROM <BD>.._V_SCHEMA
WHERE (? IS NULL OR SCHEMA LIKE UPPER(?))
ORDER BY SCHEMA;
```
...
FROM <BD>.._V_VIEW
WHERE SCHEMA = UPPER(?)
  AND (? IS NULL OR VIEWNAME LIKE UPPER(?))
ORDER BY VIEWNAME;
```
...
Vistas: `<BD>.._V_TABLE`, `<BD>.._V_TABLE_STORAGE_STAT`, `<BD>.._V_STATISTIC` (LEFT JOIN; timestamp de estadísticas)
...
SELECT t.RELTUPLES AS ROW_COUNT,
       ...
       stat.LASTUPDATETIMESTAMP AS STATS_LAST_ANALYZED
FROM <BD>.._V_TABLE t
...
WHERE SCHEMA = UPPER(?)
  AND (? IS NULL OR PROCEDURE LIKE UPPER(?))
ORDER BY PROCEDURE;
```
