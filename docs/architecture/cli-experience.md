...
> **Enmienda (2026-09-16, [ADR 0035](../adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md)).** Este documento es de referencia — describe el comportamiento vigente, no una decisión congelada en el tiempo como un ADR — así que se actualiza aquí en vez de anotarse solo al margen. Oscar decidió reducir el CLI de tres niveles a dos: **Nivel A** (estilo inline, antes "nivel 1") y **Nivel B** (plano scripteable, antes "nivel 0"). El **nivel 2 — pantalla completa — se eliminó entero**: el menú de `nz-mcp` sin argumentos (ADR 0030), el asistente de configuración de pantalla completa (ADR 0028/0029) y la pantalla "Ver perfiles" (ADR 0032) ya no existen. Donde este documento describe alguno de los tres como arquitectura vigente —la sección 3 (R3), la prohibición 1 de la sección 6 y toda la sección 10— léase como lo que fue: el estado entre 2026-09-06 y 2026-09-16, no el de hoy. `init`/`add-profile` siguen siendo el asistente guiado que las paradas 3-5 describen (ese ritmo no cambió), pero **siempre** como la cadena de preguntas de texto que esas paradas también describen — nunca como pantalla completa.
...
> **Enmienda (2026-09-10, issue #236).** Esa detección tiene ahora nombre: `cli_output.terminal_level()`, la función del [ADR 0031](../adr/0031-mejora-progresiva-por-capacidad-del-terminal.md) que responde **0** (ASCII sin secuencias de escape: el piso) o **1** (terminal moderno) con las siete señales de su punto 2, en ese orden, y con `NZ_MCP_UI_LEVEL=0|1` como única forma de forzarla en los dos sentidos (un valor inválido cuenta como ausente; `NZ_MCP_UI_LEVEL` gana a `NO_COLOR`). Ningún comando repite la comprobación. Lo que se dibuja **no cambia** con este issue: el nivel 0 queda fijado carácter a carácter en `tests/unit/test_cli_output.py`, con una aserción estructural —ni `\x1b` ni ningún carácter fuera del ASCII imprimible— que es el contrato que el nivel 1 (issue #237) tiene que respetar. `color_enabled()` y `animation_enabled()` conservan su verdad de siempre y no se reexpresan sobre el nivel. *(Hasta el ADR 0035: la misma función era el octavo disparador de `interactive_ui_blocker()`, que decidía si la pantalla completa podía abrir. Esa función y esa pantalla ya no existen.)*

> **Segunda enmienda (2026-09-11, issue #255).** El [ADR 0033](../adr/0033-preparar-consola-windows-antes-de-medir.md) hace que la señal 7 —Windows sin `WT_SESSION` y sin code page 65001— pase a evaluarse **después** de un intento de mejorar la consola: `cli_output.prepare_windows_console()` corre antes que cualquier pregunta de `terminal_level()`, en todo comando salvo `serve`, y activa el modo de terminal virtual y la code page 65001 cuando `sys.stdout` o `sys.stderr` son una consola real de Windows. Nada de lo que se dibuja cambia por esto — sigue siendo exactamente lo que el punto 5 de arriba describe—; lo que cambia es cuántas consolas de Windows llegan a nivel 1 sin que su dueño toque nada. La variable de escape es `NZ_MCP_NO_CONSOLE_PREP`, independiente de `NZ_MCP_UI_LEVEL`, y la code page y los modos de consola se restauran siempre al salir.

### R3 — ADR 0005 sigue vigente, sin excepciones activas

Sin frontend, sin UI propia, sin TUI navegable. Esto es formato y progreso en comandos puntuales. Nada de este diseño captura el teclado, dibuja pantallas ni ofrece menús navegables.

> **Historia de esta sección, resumida (vigente hasta el [ADR 0035](../adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md), 2026-09-16).** Entre el 2026-09-06 y el 2026-09-16 el ADR 0005 tuvo tres excepciones activas, abiertas una a una: el [ADR 0028](../adr/0028-asistente-de-configuracion-interactivo.md)/[0029](../adr/0029-adoptar-textual-para-el-asistente-de-configuracion.md) para el asistente de configuración de pantalla completa, el [ADR 0030](../adr/0030-menu-interactivo-como-punto-de-entrada.md) para el menú de `nz-mcp` sin argumentos, y el [ADR 0032](../adr/0032-rediseno-del-nivel-2-del-cli-interactivo.md) para la pantalla "Ver perfiles". El [ADR 0031](../adr/0031-mejora-progresiva-por-capacidad-del-terminal.md) documentó la degradación de esas tres pantallas (ocho disparadores en `interactive_ui_blocker()`, uno en cada aplicación). **El ADR 0035 retira las tres excepciones.** Hoy no queda ninguna: el ADR 0005 aplica sin salvedades, `interactive_ui_blocker()` se eliminó de `cli_output.py` por falta de consumidor, y las tres superficies citadas arriba dejaron de existir. `init`/`add-profile` usan siempre la cadena de preguntas de texto que el ADR 0028 ya tenía como camino de degradación — nunca una pantalla.
...
- El asistente gana una recapitulación antes de validar: host, puerto, base de datos, usuario y modo juntos. **Nunca la password**, ni enmascarada: la regla inviolable no admite matices y una máscara sigue siendo una invitación a comprobar por encima del hombro. *(Hasta el ADR 0035 esto hablaba solo de la recapitulación, porque el asistente de pantalla completa tenía su propio campo de contraseña con una máscara mientras tenía el foco — ADR 0029, adenda 2. Ese campo se eliminó con la pantalla; la cadena de preguntas de texto nunca mostró nada de la password, enmascarado o no.)*
...
**Sin terminal no se adivina ningún ancho.** Redirigido, canalizado o en CI se usa un valor fijo y las columnas se siguen dimensionando por su contenido, así que un archivo recibe exactamente los mismos bytes que antes de todo esto **mientras el contenido quepa en ese ancho fijo**, que es todo lo que este CLI enseña en la práctica. Un archivo se guarda y se lee después, en una ventana que no tiene nada que ver con aquélla; encogerlo a una terminal que no está sería perder datos para beneficio de nadie.

La promesa exacta importa, así que aquí está sin redondear: una celda de más de 200 caracteres —un host cerca del límite de DNS es la forma realista de tener una— **tampoco sobrevivía antes**. `rich` la cortaba por la cola y marcaba el corte con un carácter de puntos suspensivos de verdad, así que se perdía el **final** del valor y el marcador sale como `?` en una consola de Windows con página de códigos heredada. Ahora esa misma celda pierde el medio, conserva las dos puntas y lo dice en ASCII. Ninguna de las dos versiones lo conserva entero, **no son los mismos bytes**, y lo único que se afirma es que ésta es la mejor de las dos.
...
1. **Ni TUI, ni menús navegables, ni captura de teclado.** ADR 0005, sin excepciones activas desde el [ADR 0035](../adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md) (2026-09-16). El CLI se usa por SSH, dentro de contenedores y desde otros procesos; una TUI se rompe en los tres, y es exactamente la razón por la que Oscar decidió retirar las tres excepciones que este documento describía aquí entre el 2026-09-06 y el 2026-09-16 (el asistente de configuración de pantalla completa, el menú de `nz-mcp` sin argumentos y la pantalla "Ver perfiles"). Los once comandos son y siguen siendo texto puro.
...
> **Enmienda (2026-09-10; ajustada el 2026-09-16 por el [ADR 0035](../adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md)).** El [ADR 0031](../adr/0031-mejora-progresiva-por-capacidad-del-terminal.md) acota la **prohibición 4** al **Nivel B** (antes "nivel 0"): ahí los marcadores de estado siguen siendo ASCII, sin excepción. En **Nivel A** (antes "nivel 1") se permiten los **símbolos** `●`, `▲` y `✕` —símbolos, no emoji: ancho de una celda, sin secuencia de variación ni presentación en color, y presentes en cualquier fuente de terminal—, y **siempre con su palabra al lado** (`● OK`, `▲ Aviso`, `✕ Error`), de modo que la prohibición 5 se sigue cumpliendo entera. El emoji sigue prohibido en los dos niveles (ya no hay un tercero). La misma enmienda corrige la **prohibición 1**: el ADR 0005 no tiene hoy ninguna excepción activa.
...
## 10. Los dos niveles, de un vistazo

> Añadido el 2026-09-10 al cerrar el issue [#242](https://github.com/Oscarsp15/nz-mcp/issues/242); reescrito el 2026-09-16 tras el [ADR 0035](../adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md), que eliminó el nivel 2 (pantalla completa) que esta tabla enumeraba. Referencia rápida para quien documenta o da soporte; la decisión completa de los dos niveles que quedan —señales, orden de evaluación, paleta medida— vive en el [ADR 0031](../adr/0031-mejora-progresiva-por-capacidad-del-terminal.md) y no se repite aquí.

| Nivel | Qué se ve | Cuándo se activa |
|---|---|---|
| B — piso garantizado (antes "nivel 0") | Marcos ASCII, marcadores en texto (`OK`, `Aviso`, `Error`), sin color ni secuencia de escape | Terminal heredado, `NO_COLOR`, `TERM=dumb`, `CI`, salida redirigida o canalizada, o `NZ_MCP_UI_LEVEL=0` |
| A — terminal moderno (antes "nivel 1") | Color semántico, bordes redondeados, glifos junto a su palabra (`● OK`, `▲ Aviso`, `✕ Error`), indicador de actividad fluido | Terminal moderno detectado por `terminal_level()` (incluye Windows Terminal, y macOS/Linux con locale UTF-8), o forzado con `NZ_MCP_UI_LEVEL=1` |

No hay un tercer nivel. El menú de inicio, el asistente de configuración de pantalla completa y la pantalla "Ver perfiles" que esta tabla listaba como "nivel 2" se eliminaron con el ADR 0035: `nz-mcp` sin argumentos imprime la ayuda (código 2) y `init`/`add-profile` usan siempre la cadena de preguntas de texto. Ningún nivel añade ni quita información respecto al otro: cambia solo con qué se dibuja. El Nivel B es el piso garantizado, no una versión reducida — el CLI funciona igual en los dos.
...
- [adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md](../adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md) — retira las tres excepciones de pantalla completa; el estado vigente de las secciones 3 (R3), 6 (prohibición 1) y 10
