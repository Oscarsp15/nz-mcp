...
> **Enmienda (2026-09-10, issue #236).** Esa detección tiene ahora nombre: `cli_output.terminal_level()`, la función del [ADR 0031](../adr/0031-mejora-progresiva-por-capacidad-del-terminal.md) que responde **0** (ASCII sin secuencias de escape: el piso) o **1** (terminal moderno) con las siete señales de su punto 2, en ese orden, y con `NZ_MCP_UI_LEVEL=0|1` como única forma de forzarla en los dos sentidos (un valor inválido cuenta como ausente; `NZ_MCP_UI_LEVEL` gana a `NO_COLOR`). Ningún comando repite la comprobación. Lo que se dibuja **no cambia** con este issue: el nivel 0 queda fijado carácter a carácter en `tests/unit/test_cli_output.py`, con una aserción estructural —ni `\x1b` ni ningún carácter fuera del ASCII imprimible— que es el contrato que el nivel 1 (issue #237) tiene que respetar. `color_enabled()` y `animation_enabled()` conservan su verdad de siempre y no se reexpresan sobre el nivel. La misma función es el **octavo disparador** de `interactive_ui_blocker()`: la pantalla completa exige nivel 1, así que `NO_COLOR`, `CI`, `NZ_MCP_UI_LEVEL=0` y el resto de señales del nivel 0 la cierran y activan la degradación del ADR 0028.
...
>
> **Tercera enmienda (2026-09-09).** El [ADR 0032](../adr/0032-rediseno-del-nivel-2-del-cli-interactivo.md) autoriza una **tercera superficie de pantalla completa** y la nombra: la pantalla **"Ver perfiles"**. Es la última que ese ADR abre —una cuarta exige su propio ADR— y **las prohibiciones del §6 de este documento se aplican también dentro de ella**, igual que dentro del asistente y del menú. Como el menú, elige y no hospeda: la pantalla se cierra, el comando equivalente corre en la terminal de siempre y el proceso termina con su código. Y el texto de esa ruta pasa a formato **`Etiqueta: valor`** —etiqueta, dos puntos, valor alineado en columna—, que es lo que fija la decisión 7 de aquel ADR.
>
> **Cuarta enmienda (2026-09-10).** El [ADR 0031](../adr/0031-mejora-progresiva-por-capacidad-del-terminal.md) corrige dos recuentos de las enmiendas anteriores. El ADR 0005 sigue vigente **salvo las excepciones que sus ADR abren** —ya no son dos: el ADR 0032 añade una tercera—. Y la degradación de la pantalla completa la deciden **siete disparadores en la función `interactive_ui_blocker()` y uno en la aplicación** (encoger la ventana a mitad de sesión), **más el octavo de la función que añade el ADR 0031**: `terminal_level() == 0`, que la cierra allí donde el terminal no da ni para el nivel 1.
...
1. **Ni TUI, ni menús navegables, ni captura de teclado.** ADR 0005. Y el CLI se usa por SSH, dentro de contenedores y desde otros procesos; una TUI se rompe en los tres. **Acotado desde el 2026-09-06 por el [ADR 0028](../adr/0028-asistente-de-configuracion-interactivo.md)**, que abre la excepción para el asistente de configuración —y solo para él— con la degradación como requisito: los tres escenarios de arriba son precisamente los que la disparan. Y **acotado una segunda vez, el mismo día, por el [ADR 0030](../adr/0030-menu-interactivo-como-punto-de-entrada.md)**, que permite un **menú** en un sitio y solo en uno: `nz-mcp` sin argumentos, donde antes se imprimía esta ayuda. No es un comando el que gana pantalla, es el hueco que queda cuando no se escribe ninguno; los once comandos siguen siendo texto puro y el menú los **lanza**, con la ayuda de siempre como caída ante cualquiera de los ocho disparadores. Las otras doce prohibiciones de esta lista se aplican también dentro del asistente y dentro del menú.
...
## 10. Los tres niveles, de un vistazo

> Añadido el 2026-09-10 al cerrar el issue [#242](https://github.com/Oscarsp15/nz-mcp/issues/242). Referencia rápida para quien documenta o da soporte; la decisión completa —señales, orden de evaluación, paleta medida— vive en el [ADR 0031](../adr/0031-mejora-progresiva-por-capacidad-del-terminal.md) y no se repite aquí.
