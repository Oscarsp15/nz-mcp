...
- **Guard de coste** (issue #361): sin `database`, la búsqueda **no se ejecuta** si el patrón no acota nada (solo comodines `%`/`_`, que coinciden con todo). Devuelve `INPUT_TOO_BROAD` **antes de abrir conexión** —no barre ninguna base— con un hint que nombra `database` como escape o un patrón con algún carácter literal. Un patrón con carácter literal, aunque no se indique `database`, sigue recorriendo todas las visibles como siempre: medido en vivo, el entorno SaaS tiene 62 bases visibles y un tope de bases habría rechazado justo esa búsqueda.
