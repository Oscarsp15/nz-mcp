...
| `database` | string (optional) | Limita la búsqueda a una base; si se omite, recorre todas las visibles **salvo que el patrón no acote nada** (ver Reglas). |
| `schema_pattern` | string (optional) | Filtro `LIKE` sobre el esquema. Match case-insensitive. |
...
- **Guard de coste** (issue #361): sin `database`, la búsqueda **no se ejecuta** si el patrón no acota nada (solo comodines `%`/`_`, que coinciden con todo). Devuelve `INPUT_TOO_BROAD` **sin abrir ninguna conexión** —no lista las bases ni las barre— con un hint que nombra `database` como escape o un patrón con algún carácter literal. Un patrón con carácter literal, aunque no se indique `database`, sigue recorriendo todas las visibles como siempre: medido en vivo, el entorno SaaS tiene 62 bases visibles y un tope de bases habría rechazado justo esa búsqueda.
