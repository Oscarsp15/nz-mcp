...
- **No enmienda el [ADR 0005](../adr/0005-sin-frontend.md).** Sigue sin haber frontend, UI propia ni TUI navegable, y **el rol no puede cambiar eso**. Las dos excepciones que existían —el asistente de configuración ([ADR 0028](../adr/0028-asistente-de-configuracion-interactivo.md)) y el menú de entrada ([ADR 0030](../adr/0030-menu-interactivo-como-punto-de-entrada.md))— quedaron recogidas por el [ADR 0035](../adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md): el menú se eliminó y el asistente perdió su pantalla completa, corriendo siempre por el camino de texto. El modelo vigente es de **dos niveles** (Nivel B — ASCII sin color; Nivel A — terminal moderno con estilo), sin pantalla completa en ningún sitio. Fuera de esos dos niveles, ampliar este rol a la audiencia humana sigue siendo formato, progreso y redacción en comandos puntuales. Cualquier nuevo nivel o superficie necesita su propio ADR.
...
3. **Sin frontend ni TUI navegable.** El [ADR 0035](../adr/0035-cli-de-dos-niveles-sin-pantalla-completa.md) formaliza el modelo vigente: dos niveles (Nivel B — ASCII sin color; Nivel A — terminal moderno con estilo), sin pantalla completa en ningún sitio. El asistente de configuración ([ADR 0028](../adr/0028-asistente-de-configuracion-interactivo.md), enmendado por el ADR 0035) corre siempre por el camino de texto. En todos los comandos: formato y progreso, nunca un interfaz que capture el teclado.
