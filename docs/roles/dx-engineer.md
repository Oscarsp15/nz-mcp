...
- **No enmienda el [ADR 0005](../adr/0005-sin-frontend.md).** Sigue sin haber frontend, UI propia ni TUI navegable, y **el rol no puede cambiar eso**. Lo cambió un ADR: el [0028](../adr/0028-asistente-de-configuracion-interactivo.md) abre una excepción **para un solo comando**, el asistente de configuración, con la degradación como requisito y la librería decidida aparte en el [0029](../adr/0029-adoptar-textual-para-el-asistente-de-configuracion.md). Y el [0030](../adr/0030-menu-interactivo-como-punto-de-entrada.md) abre la segunda y última hasta hoy, que **no es un comando**: es `nz-mcp` sin argumentos, donde antes se imprimía la ayuda. Fuera de esos dos sitios, ampliar este rol a la audiencia humana sigue siendo formato, progreso y redacción en comandos puntuales. Cualquier comando que quiera interfaz necesita su propia enmienda; ninguno de los dos precedentes sirve de jurisprudencia.
...
3. **Sin frontend ni TUI navegable salvo en dos sitios**, y solo en las condiciones de sus ADR: el asistente de configuración —[ADR 0028](../adr/0028-asistente-de-configuracion-interactivo.md), que enmienda el [ADR 0005](../adr/0005-sin-frontend.md) para `nz-mcp init` y la rama interactiva de `add-profile`, que son el mismo código— y el menú de `nz-mcp` **sin argumentos** —[ADR 0030](../adr/0030-menu-interactivo-como-punto-de-entrada.md), que no le da pantalla a ningún comando: los lanza—. En todos los comandos: formato y progreso, nunca un interfaz que capture el teclado. Y las dos excepciones vienen con la misma obligación: si el entorno no lo soporta, **degradan** —al camino de texto el asistente, a la ayuda de siempre el menú— y sirven igual de bien.
