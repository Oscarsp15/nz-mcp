...
If your terminal cannot host that screen — input or output redirected, `TERM=dumb`, an empty or unknown `TERM` on POSIX (routine inside containers), a Windows console without VT sequences, or a window smaller than 60x21 — the wizard **falls back on its own** to the same eight chained questions as always, with no warning and no error, and configures exactly the same profile. If you would rather always have the questions, set `NZ_MCP_NO_TUI=1` in your shell. And if you shrink the window halfway through, it switches to the questions **keeping everything you had already typed**.
